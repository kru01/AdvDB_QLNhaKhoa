...
RETURNS BIT AS BEGIN
    IF EXISTS (SELECT IDNHASI FROM LICHHEN WHERE IDNHASI = @IDNHASI
        AND NGAY = @NGAY AND ABS(GIO - @GIO) <= 60)
    OR EXISTS (SELECT IDTROKHAM FROM LICHHEN WHERE IDTROKHAM = @IDNHASI
        AND NGAY = @NGAY AND ABS(GIO - @GIO) <= 60) RETURN 0

    IF EXISTS (SELECT IDNHASI FROM LICHNGAY WHERE IDNHASI = @IDNHASI
        AND NGAY = @NGAY AND GIOBATDAU <= @GIO AND @GIO <= GIOKETTHUC)
        RETURN 1

    IF EXISTS (SELECT IDNHASI FROM LICHTUAN WHERE IDNHASI = @IDNHASI
        AND NGAYTRONGTUAN = DATEPART(WEEKDAY, @NGAY)
        AND GIOBATDAU <= @GIO AND @GIO <= GIOKETTHUC) RETURN 1

    IF EXISTS (SELECT IDNHASI FROM LICHTHANG WHERE IDNHASI = @IDNHASI
