...
AS BEGIN TRAN
    ...
    IF @IDNHANVIENDAT IS NOT NULL AND NOT EXISTS (SELECT IDTAIKHOAN
        FROM TAIKHOAN WHERE IDTAIKHOAN = @IDNHANVIENDAT) BEGIN
...
CREATE OR ALTER PROC USP_LICHHEN_DEL
    ...
    @IDLICHHEN CHAR(7)
AS BEGIN TRAN
    IF NOT EXISTS (SELECT IDLICHHEN FROM LICHHEN WHERE IDHOSO = @IDHOSO
        AND IDLICHHEN = @IDLICHHEN) BEGIN
        RAISERROR('INVALID LICHHEN', 16, 1)
        ROLLBACK TRAN
        RETURN -1
    END

    DELETE FROM LICHHEN WHERE IDHOSO = @IDHOSO AND IDLICHHEN = @IDLICHHEN
COMMIT TRAN
RETURN 0
GO